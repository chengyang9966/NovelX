...
const ForgetPassword = function (app) {
  app.post('/api/forgetpassword', (req, res) => {
    Query('SELECT email,id FROM users WHERE email=$1', [req.body.email]).then(
      (respone) => {
        if (respone.length > 0) {
          ...
          SendEmail(
            ...
            Query('Update users SET resetpassword=true WHERE email=$1', [
              req.body.email
            ]).then((responeQuery) => {
              res.status(200).json({
  ...
  app.post('/api/verifyReset', Auth, (req, res) => {
    Query('SELECT resetpassword FROM users WHERE id=$1', [req.body.userid])
      .then((respone) => {
      ...
      .catch((err) => res.status(419).json({ message: 'Link Expired' }));
  });

  app.post('/api/resetpassword', Auth, (req, res) => {
    let { email, password } = req.body;
    Query('SELECT resetpassword FROM users WHERE  email=$1', [email]).then(
      (respone) => {
        if (respone[0].resetpassword == true) {
          HashPassword(password).then((responses) => {
            Query(
              'UPDATE users SET password=$1,resetpassword=$2 where email=$3',
              [responses, false, email]
            )
              .then((respone) => {
                res.status(200).json({
