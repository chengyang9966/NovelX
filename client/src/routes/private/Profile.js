...
const ProfilePage = () => {
  console.log(
    "CountryList: ",
    CountryList.findIndex((w) => w === "Malaysia")
  );
  ...
  let userid = DescrytionObj("user").UserId;
  useEffect(() => {
    setLoading(true);
    axios
      .get(`/api/getusercontact/${userid}`, config)
      .then((res) => {
  ...
  const onSubmit = (e) => {
    ...
    if (Checker(Newdata).Error === true) {
      ...
      axios
        .post(`/api/updateusercontact/${userid}`, Newdata, config)
        .then((res) => {
          setLoading(false);
          SetcardTextForTitle({
            ...cardTextForTitle,
  ...
  return (
    <>
        ...
        <div>
          ...
          <form onSubmit={(data) => onSubmit(data)}>
            ...
            <div className="row  px-4 ">
              <div className=" col text-danger ">{error.FnameText}</div>
              <div className="text-danger col">{error.LnameText}</div>
            </div>
            <div className="row mb-3 px-4">
              <div className="col">
                <label for="exampleInputEmail1" className="form-label">
      ...
      {openCard && (
        <Card
          title={cardTextForTitle.title}
          setClose={CardText.setClose}
          description={cardTextForTitle.description}
        />
      )}
    </>
  );
